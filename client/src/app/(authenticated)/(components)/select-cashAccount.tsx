...
export function SelectCashAccount({ t, children, onSelect }: Props) {
      ...
      <AlertDialogContent className=" px-3 py-1 lg:max-w-[900px]">
        ...
        <div className="overflow-y-auto h-[600px]">
          ...
          <div className='grid grid-cols-1 w-full text-xs lg:text-sm border-b my-2 ps-2 pe-5'>
            <div className=''>{t('CashAccount')}</div>
          </div>
...
export function SelectCashAccountWithLabel({ t, children, onSelect, className, caption }: WithLabelProps) {
  return (
    <div className={`w-full flex justify-between p-2 pe-4 items-start  border rounded-md border-dashed ${className}`}>
      <div className="flex flex-col gap-1">
        <Label className="text-muted-foreground">{t('Cash Account')}</Label>
        <div className="capitalize">{caption}</div>
      </div>
      <SelectCashAccount t={t} onSelect={onSelect} ><ButtonSelect /></SelectCashAccount>
    </div>
  )
}